...
namespace Daftari.Services.REST.Helpers
{
    ...
    {
        public static async Task<SimpleResult> Login(object user, CancellationTokenSource ct_Token)
        {
            var tcs = new TaskCompletionSource<SimpleResult>();

            try
            {
                var deroResult = await new RestService().PostAsync((Application.Current as App), Constants.URLs.Login, user, ct_Token.Token);

                if (deroResult != null)
                {
                    if (deroResult.IsSuccess)
                    {
                        tcs.SetResult(new SimpleResult { IsSuccess = true, Body = deroResult.Body });
                    }
                    else
                    {
