...
namespace Daftari.Controllers
{
    public class StudentCardsController : Controller
    {
        ...
        //public string CleanUp10()
        //{
        //    db.Configuration.AutoDetectChangesEnabled = false;
        //    db.TrackingEnabled = false;
        //    var studentCards = db.StudentCards.Include(s => s.StudentCardDetails).ToList();

        //    foreach (var card in studentCards)
        //    {
        //        if (!card.StudentCardDetails.Any())
        //        {
        //            db.StudentCards.Remove(card);
        //        }
        //        else if (card.StudentCardDetails.All(d => !d.IsComplete))
        //        {
        //            db.StudentCards.Remove(card);

        //            foreach (var item in card.StudentCardDetails)
        //            {
        //                db.StudentCardDetails.Remove(item);
        //            }
        //        }
        //    }
        ...
        //public string CleanUp20()
        //{
        //    db.Configuration.AutoDetectChangesEnabled = false;
        //    db.TrackingEnabled = false;
        //    var customersWithCards = db.StudentCards.Include(s => s.Customer).Select(c => c.Customer).Distinct().ToList();

        //    var allCustomers = db.Customers.ToList();

        //    foreach (var c in customersWithCards)
        //    {
        ...
        [Authorize(Roles = "Admin,Manager")]
        public async Task<JsonResult> GetJSON(int filter = 0)
        {
            var query = db.GetCards();
            IEnumerable<StudentCard> cards = new List<StudentCard>();

            switch (filter)
            {
                case 0:
                    cards = (await query.Where(c => !c.IsGraduated).ToListAsync()).Where(c => !c.IsExpired).ToList();
                    break;

                case 1:
                    cards = await query.Where(c => c.IsGraduated).ToListAsync();
                    break;
