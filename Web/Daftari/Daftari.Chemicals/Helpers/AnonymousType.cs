...
namespace Daftari.Chemicals.Helpers
{
    public static class AnonymousType
    {
                ...
                lock (GeneratedTypes)
                {
                    if (!GeneratedTypes.TryGetValue(fullName, out type))
                    {
                        ...
                        for (int i = 0; i < names.Length; i++)
                        {
                            ...
                            PropertyBuilder property = tb.DefineProperty(names[i], System.Reflection.PropertyAttributes.None, CallingConventions.HasThis, generics[i], Type.EmptyTypes);
        ...
        public static object FromDictonaryToAnonymousObject<TValue>(IDictionary<string, TValue> dict)
        {
